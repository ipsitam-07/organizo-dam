...
export function SignUpPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setErrors({ form: (err as ApiError).message ?? "Registration failed" });
    }
  };
        ...
        <div className="border-border-light rounded-2xl border bg-white p-6 shadow-sm dark:border-white/5 dark:bg-black/20 dark:backdrop-blur-xl">
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {strength && (
                <div className="mt-0.5 flex items-center gap-2">
                  ...
                  <span className="text-text-secondary w-10 shrink-0 text-right text-[11px]">
                    {strength.label}
                  </span>
                </div>
              )}
            ...
            <Button type="submit" size="lg" className="mt-1 w-full">
              {UI_STRINGS.BUTTONS.CREATE}
            </Button>
          </form>
          <p className="text-text-secondary mt-4 text-center text-[12px] leading-relaxed">
            {UI_STRINGS.SIGN_UP_PAGE.AGREE}{" "}
            <span className="text-text-primary font-medium dark:text-white">
